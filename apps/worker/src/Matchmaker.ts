...
export class Matchmaker extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      return new Response(error instanceof Error ? error.message : "Invalid session", { status: 400 });
    }
  ...
  async webSocketMessage(socket: WebSocket, value: string | ArrayBuffer): Promise<void> {
    try {
      ...
      sendReply(socket, undefined, {
        ok: false,
        message: "Invalid queue message."
      });
    }
  }
  ...
  private async match(mode: GameMode, queue: QueueEntry[]): Promise<void> {
    ...
    try {
      await room.initializePublic(
        roomId,
        mode,
        members.map(({ playerId, reconnectToken, name }) => ({
          playerId,
          reconnectToken,
          name
        }))
      ...
      await this.ctx.storage.put("queue", [
        ...members,
        ...current.filter((entry) => !members.some((member) => member.playerId === entry.playerId))
      ]);
      console.error(
        JSON.stringify({
          level: "error",
    ...
    for (const socket of this.ctx.getWebSockets()) {
      ...
      if (attachment && matchedIds.has(attachment.playerId)) {
        ...
        socket.serializeAttachment({ ...attachment, matched: true } satisfies QueueAttachment);
  ...
  private async remove(socket: WebSocket): Promise<void> {
    const attachment = socket.deserializeAttachment() as QueueAttachment | null;
    if (!attachment?.playerId || attachment.matched) return;
    const queue = (await this.ctx.storage.get<QueueEntry[]>("queue")) ?? [];
