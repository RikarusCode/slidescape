...
interface CreatePrivateBody extends SessionIdentity { mode: GameMode; turnTimerSeconds: TurnTimerSeconds }
interface ModeBody extends SessionIdentity { mode: GameMode }

async function body<T>(request: Request): Promise<T> {
  if (!request.headers.get("content-type")?.includes("application/json")) throw new Error("Expected a JSON request.");
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/private/create") {
        ...
        for (let attempt = 0; attempt < 12; attempt += 1) {
          ...
          const lobby = await env.GAME_ROOMS.getByName(code).initializePrivate(code, code, identity, { mode: input.mode, turnTimerSeconds: privateTimer(input.turnTimerSeconds) });
          if (lobby) return json({ ok: true, roomId: code, lobby });
        }
        return json({ ok: false, message: "Could not reserve a private room code. Try again." }, 503);
      }
      ...
      if (request.method === "POST" && url.pathname === "/api/private/join") {
        ...
        if (!/^[A-Z2-9]{6}$/.test(code)) return json({ ok: false, message: "Enter a valid six-character room code." }, 400);
      ...
      if (request.method === "POST" && url.pathname === "/api/bot") {
        ...
        const initialized = await env.GAME_ROOMS.getByName(roomId).initializeBot(roomId, input.mode, identity);
        return json({ ok: true, roomId, lobby: initialized.lobby, game: initialized.game });
      }
      ...
      if (request.method === "POST" && url.pathname === "/api/reconnect") {
        ...
        if (!roomId || roomId.length > 64) return json({ ok: false, message: "Your previous match is no longer available. You can start a new game." });
        const available = await env.GAME_ROOMS.getByName(roomId).validateSession(identity);
        return json(available ? { ok: true, roomId } : { ok: false, message: "Your previous match is no longer available. You can start a new game." });
      }

      if (url.pathname.startsWith("/api/") || url.pathname.startsWith("/ws/")) return json({ ok: false, message: "Not found." }, 404);
      return env.ASSETS.fetch(request);
    } catch (error) {
      console.error(JSON.stringify({ level: "error", message: "request_failed", path: url.pathname, error: error instanceof Error ? error.message : String(error) }));
      return json({ ok: false, message: error instanceof Error ? error.message : "The request failed." }, 400);
    }
  }
