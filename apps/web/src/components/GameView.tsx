import { ChevronDown, Copy, Dice1, Dice2, Dice3, Dice4, Dice5, Dice6, Fish, LogOut, Settings, Signal, Users, X } from "lucide-react";
import { FISH_CARDS, legalMoves, legalMovesForPiece, PLAYER_COLOR_HEX, POOP_CARDS, type CardReveal, type ClientCommand, type GameState, type FishPlay, type LegalMove, type Position } from "@slidescape/game";
...
export function GameView({ state, playerId, roomCode, connected, message, send, onLeaveGame, onReturnHome }: { state: GameState; playerId: string; roomCode?: string; connected: boolean; message?: string; send: (command: ClientCommand) => void; onLeaveGame: () => void; onReturnHome: () => void }) {
  ...
  const stealTargets = state.players.filter((player) => player.id !== playerId && player.fishCard);
  const specialSelectableIds = specialMode === "opponent"
    ? state.pieces.filter((piece) => !piece.scored && piece.ownerId !== playerId && piece.kind !== "walrus" && legalMovesForPiece(state, piece.id).length > 0).map((piece) => piece.id)
    : undefined;
  ...
  const selectBoardPiece = (pieceId: string) => {
    ...
    if ((specialMode === "walrus-poop" || specialMode === "walrus-only") && piece?.kind === "penguin") clearSpecial();
  ...
  const playSimpleCard = () => {
    ...
    } else if (me.fishCard === "move-opponent") { setSpecialMode("opponent"); return; }
  ...
  const chooseEmptyCell = (position: Position) => {
    ...
    if (specialMode === "walrus-poop") {
      ...
      dispatch({ type: "place-walrus", to: position, leavePoop: true, poopFrom: selectedPoop });
    }
    if (specialMode === "poop" && selectedPoop) dispatch({ type: "play-fish", play: { cardId: "relocate-and-roll", poopFrom: selectedPoop, poopTo: position } });
    clearSpecial();
  };
  ...
  return <div className="game-shell">
    ...
    <main className="game-layout"><Board state={state} playerId={playerId} selectedId={selectedId} onSelect={selectBoardPiece} specialMoves={specialMode === "opponent" && selectedId ? legalMovesForPiece(state, selectedId) : undefined} specialSelectableIds={specialSelectableIds} onMove={(move: LegalMove) => { if (specialMode === "opponent") { dispatch({ type: "play-fish", play: { cardId: "move-opponent", move } }); clearSpecial(); } else dispatch({ type: "move", move }); setSelectedId(undefined); }} onPoopSelect={isMyTurn && (specialMode === "poop" || (specialMode === "walrus-poop" && state.poopSupply === 0)) ? setSelectedPoop : undefined} selectedPoop={selectedPoop} onEmptyCell={isMyTurn && specialMode && specialMode !== "opponent" ? chooseEmptyCell : undefined}/>
      <aside className="game-sidebar">
        <section className="turn-panel"><div className="turn-title"><span className="player-token" style={{ background: PLAYER_COLOR_HEX[active.themeColor] }}/><h1>{isMyTurn ? "Your turn" : `${active.name}’s turn`}</h1></div><div className="roll-row"><div className={`die ${rolling ? "rolling" : ""}`} aria-label={rolling ? "Rolling die" : state.turn.rolled ? `Die shows ${state.turn.rolled}` : "Die ready"}><DieIcon/></div><div>{rolling ? <><strong>Rolling…</strong><span>The die is tumbling</span></> : state.turn.rolled ? <><strong>Rolled {state.turn.rolled}</strong><span>{state.turn.movesRemaining} {state.turn.movesRemaining === 1 ? "move" : "moves"} left</span></> : <><strong>Ready to roll</strong><span>{isMyTurn ? "Your move" : "Waiting"}</span></>}</div></div>
          ...
          {isMyTurn && state.turn.fishDrawAvailable && !me.fishCard ? <button className="fish-action" onClick={() => dispatch({ type: "draw-fish" })}><Fish size={18}/> Take a Fish card instead</button> : null}
    ...
    {visibleReveal && revealCard ? <section className="card-reveal" role="dialog" aria-label="Poop card drawn">
      ...
      {canAvoidVisiblePoop ? <button className="card-reveal-action" onClick={() => { dispatch({ type: "play-fish", play: { cardId: "avoid-or-two", choice: "avoid" } }); setRevealQueue((queue) => queue.slice(1)); }}><Fish size={17}/> Use Fish card to avoid this</button> : null}
    ...
    {state.status === "finished" ? <GameResults state={state} playerId={playerId} onHome={onReturnHome}/> : null}
  </div>;
}
