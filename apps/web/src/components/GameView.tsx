import { ChevronDown, Copy, Dice5, Fish, Settings, Users } from "lucide-react";
import { HARVEST_CARDS, legalMoves, legalMovesForPiece, type ClientCommand, type GameState, type HarvestPlay, type LegalMove, type Position } from "@slidescape/game";
...
export function GameView({ state, playerId, roomCode, connected, send }: { state: GameState; playerId: string; roomCode?: string; connected: boolean; send: (command: ClientCommand) => void }) {
  ...
  return <div className="game-shell">
    <header className="game-header"><div className="wordmark"><span className="penguin-mark"/>Slidescape</div><div className="room-meta"><span>{roomCode ? "Private game" : state.mode.replace("-", " ")}</span>{roomCode ? <button onClick={() => navigator.clipboard.writeText(roomCode)}>Room {roomCode}<Copy size={15}/></button> : null}</div><div className="connection"><i className={connected ? "online" : "offline"}/>{connected ? "Connected" : "Reconnecting"}<RulesButton/><button aria-label="Settings"><Settings size={21}/></button></div></header>
    <main className="game-layout"><Board state={state} playerId={playerId} selectedId={selectedId} onSelect={setSelectedId} specialMoves={specialMode === "opponent" && selectedId ? legalMovesForPiece(state, selectedId) : undefined} onMove={(move: LegalMove) => { if (specialMode === "opponent") { dispatch({ type: "play-harvest", play: { cardId: "move-opponent", move } }); clearSpecial(); } else dispatch({ type: "move", move }); setSelectedId(undefined); }} onPoopSelect={specialMode === "poop" || (specialMode === "walrus-poop" && state.poopSupply === 0) ? setSelectedPoop : undefined} selectedPoop={selectedPoop} onEmptyCell={specialMode && specialMode !== "opponent" ? chooseEmptyCell : undefined}/>
      <aside className="game-sidebar">
        <section className="turn-panel"><div className="turn-title"><span className={`player-token ${active.colors[0]}`}/><h1>{isMyTurn ? "Your turn" : `${active.name}’s turn`}</h1></div><div className="roll-row"><div className="die"><Dice5/></div><div>{state.turn.rolled ? <><strong>Rolled {state.turn.rolled}</strong><span>{state.turn.movesRemaining} {state.turn.movesRemaining === 1 ? "move" : "moves"} left</span></> : <><strong>Ready to roll</strong><span>{isMyTurn ? "Your move" : "Waiting"}</span></>}</div></div>
          ...
          {isMyTurn && state.turn.phase === "awaiting-roll" && active.effects.forcedOpponentMoves === 0 ? <button className="primary-action" onClick={() => dispatch({ type: "roll" })}>Roll the die</button> : null}
