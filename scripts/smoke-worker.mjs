...
function connection(path) {
  ...
  const opened = new Promise((resolve, reject) => {
    ...
    socket.addEventListener(
      ...
      () => {
        clearTimeout(timer);
        reject(new Error(`WebSocket failed: ${path}`));
      },
      { once: true }
    );
  });
  ...
  function send(event, payload) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        replies.delete(id);
        reject(new Error(`Reply timed out: ${event}`));
      }, timeoutMs);
...
const created = await post("/api/private/create", {
  ...host,
  mode: "quick-2",
  turnTimerSeconds: 45
});
...
const [hostLobby, guestLobby] = await Promise.all([
  hostRoom.next("lobby-state"),
  guestRoom.next("lobby-state")
]);
assert(
  hostLobby.members.length === 2 && guestLobby.members.length === 2,
  "Private lobby did not converge to two players."
);

const hostColor = await hostRoom.send("select-color", { color: "berry-pink" });
const duplicateColor = await guestRoom.send("select-color", {
  color: "berry-pink"
});
...
assert(
  hostGame.fishDeck.length === 0 && hostGame.poopDeck.length === 0,
  "Server-only deck order leaked to a client."
);
...
assert(
  restored.id === hostGame.id && restored.version === hostMoved.version,
  "Room reconnect did not restore canonical state."
);
...
const [firstMatch, secondMatch] = await Promise.all([
  firstQueue.next("matched"),
  secondQueue.next("matched")
]);
...
assert(
  publicLobby.members.length === 2 && publicLobby.settings.turnTimerSeconds === 90,
  "Public lobby settings are incorrect."
);

for (const active of [guestRoom, hostReconnect, publicOne, publicTwo])
  active.socket.close(1000, "Smoke complete");
console.log(
  JSON.stringify({
    ok: true,
    privateCode: created.roomId,
    publicRoomId: firstMatch.roomId,
    checks: 11
  })
);
